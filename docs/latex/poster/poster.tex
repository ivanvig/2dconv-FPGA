...
%
{  
  {\includegraphics[width=\textwidth]{DSP-02}\vspace{0.5em}} % SASE logo
  \bf\textsf %Sans Serif  
...
\headerbox{2. Method}{name=model,column=0,below=introduction,span=1}{
\footnotesize{
...
the described transformation in Section ??? using a Python script.
In addition, the script divides the image into batches and sends them through
the UART port to the FPGA. A batch consists of contiguous pixel columns whose
dimension is described in Section ???. In the FPGA a microprocessor receives the information
...
\headerbox{4. Results}{name=sea,span=2,column=1,below=screen}{ % To reduce this block to 1 column width, remove 'span=2'
