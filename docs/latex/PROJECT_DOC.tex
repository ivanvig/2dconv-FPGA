...
processing is ended a notification will be emitted through Control Block's
...
that reduces even more the memory use and the MMU are explained.
...
that already is in the memory, and algorithm was designed. Given $N$ convolution
units and a $k\times k$ kernel, a $N+k-1$ width batch is needed, but the
processed batch has a width of $N$ ,i.e. one for each convolution unit, this
leaves the last $k-1$ memory columns with the input data, because they weren't
overwritten. So this $k-1$ columns will be reused as the first columns of the
next batch, therefore the batch width is reduced to $N$, with the exception of
...
that in every iteration the position of the memory columns asociated with each
