...
\author{\IEEEauthorblockN{
  ...
  \IEEEauthorblockA{
    xxxxxxxxxx xx xxxxxxxx xxx xxxxxxxxxxx\\
    xxxxxxxxx xxxxxx \\
    xxxxxxx xxxxxxxx xxx, xxxxxxx xxxxxxxx, xxxxxxxxx\\
    xxxxx: xxxxxx.xxxxxxxxx@xxxxx.xxx, xxx.xxxx@xxxxx.xxx,
    xxxxxxxx@xxxxx.xxx, xxxxxxxxx@xxxxx.xxx}
}
%\author{\IEEEauthorblockN{
    %Martin Casabella,
    %Sergio Sulca,
...
\begin{abstract}
  This paper presents a hardware architecture for two-dimensional (2D)
  convolution implementation on a Xilinx Arty 7 FPGA platform when there is not
  enough RAM memory to instantiate and store a complete image. Not only
  processing speed is prioritized, but also efficient resource utilization and
  scalability in design, so as to add as many convolution operations in parallel
  as needed without requiring any major changes. In the proposed structure,
 dynamic memory reuse is accomplished, with a linear increase in memory utilization
...
Filtering process is widely used in many applications, such as aeronautics, navigation,
...
The images used for processing are grayscale because of its implementation simplicity, but the design can be extrapolated to any other type of image.
...
batch, the proposed approach is to organize the block of RAM in columns. An image batch
...
sum every term. Finally, it truncates the result and then stores it into the first position of the memory.

Once a pixel is processed, it is stored in the memory while the MAC unit shifts
its image register, discarding the previous $k$ pixels and loading new $k$ pixels,
i.e. one from each input column, and this is equivalent to a first input first output (FIFO) structure.
...
column in a MAC unit, that means that $2 \times k$ columns are needed for two MAC units.
...
overlap between the two inputs and this causes that despite $k$ input columns are
still needed per MAC unit, only $k+1$ different input columns are needed for all
the units. For this reason, MAC units in parallel produce contiguous processed
columns, sharing the common input columns. Therefore, for $N$ MAC units, the number
of required memory columns is reduced from $N \times k$ to $N+k-1$, concluding
that adding a new MAC unit only adds one new memory column.

As a consequence of the overlap explained, repeated data is being used
...
follows that a periodicity in the ratio between the memory columns and the
...
inputs ratio, i.e. when $It(k-1)$ is a multiple of $N+k-1$. That is, there
...
$k=3$ and 6 memory columns. In the first iteration, the memories 1 to 6 are
loaded with the new batch, and the result is stored in memories 1 to 4. In the next
iteration, a new batch is loaded without overwriting memories 5 and 6. After
processing, the data is stored in memories 5 to 8. 
...
\caption{On the left, an image processed in a CPU using python and on the right the
  same image processed with the FPGA module. The filters used are \protect\subref{fig:identity}
  Identity Kernel, \protect\subref{fig:share} Sharpening Kernel and \protect\subref{fig:embossing} Embossing Kernel.}
\label{images_py_po}
\end{figure}
...
of BRAM resources. As a result, a linear behaviour regarding resource utilization increase was
achieved.
...
%The authors would like to thank PhD. Mario R. Hueda and PhD. Damian Morero for supporting their
...
%%%% Para cuando vaya a incluirse: esto es acknowledgment
%%
%%The authors would like to thank PhD. Mario R. Hueda and PhD. Damian Morero for supporting their
%%design through Fulgor fundation research program.
