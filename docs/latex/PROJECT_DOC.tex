...
applying parallelization as well as reusing resources to reduce complexity
\cite{paper1,paper5}. On the other hand, architectures with partial storage
partition the image into as many parts as convolders are used
\cite{paper2,paper4}. The difficulty presented by these schemes is the
...
\begin{equation}\label{conv-org1}
  G^\prime(x,y) = \sum_{i=0}^{m-1} \sum_{j=0}^{n-1}K^\prime(i,j)I^\prime(x-i,y-j).
\end{equation}

Finally, we apply the change of dynamic range on the output
$\mathcal{T}[G^\prime(x,y)]=G(x,y)$ again and obtain range between $[0,255]$.
...
of the module and sends it to the PC. Finally, the post-processor stage joins
...
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{muxes}
\caption{Data flow between the MACs and the storage.}
\label{muxes}
\end{figure}
...
\begin{figure}[!t]
\centering
\subfloat[Amount of memory required.]{\includegraphics[width=2.5in]{mem_space2}%
\label{mem}}
\hfil
...
the FPGA module. The kernels used were sharpening $\begin{smallmatrix}0 & -1 &
  0\\ -1 & 5 & -1\\ 0 & -1 & 0\end{smallmatrix}$,
embossing $[-2\quad-1\quad0; -1\quad0\quad1; 012]$ and identity $[0\quad0\quad0; 0\quad1\quad0; 0\quad0\quad0]$ and
the results are shown in Fig. \ref{images_py_po}.

\begin{figure}[!t]
\centering
\subfloat[Amount of memory required.]{\includegraphics[width=2.5in]{identity_c}%
\label{mem}}
\hfil
\subfloat[Amount of data transmitted for an image of $1600\times1024$px and a $3\times3$ kernel.]{\includegraphics[width=2.5in]{shaped_c}%
\label{transmitted}}
\hfil
\subfloat[Amount of data transmitted for an image of $1600\times1024$px and a $3\times3$ kernel.]{\includegraphics[width=2.5in]{emboss_c}%
\label{transmitted}}
...
microprocessor on the FPGA with and without DSP respectively. 
...
\hline
   & \multicolumn{3}{|c|}{With DSP(\%)} & \multicolumn{3}{|c|}{Without DSP(\%)}\\
\hline
  P & DSP & LUT & BRAM & DSP & LUT & BRAM\\
\hline
  2 & 22.22 & 8.87 & 20 & ----- & 22.25 & 22\\
\hline
  4 & 44.44 & 9.72 & 22 & ----- & 37.29 & 26\\
\hline
  6 & 66.67 & 10.46 & 24 & ----- & 52.49 & 30\\
\hline
  8 & 88.89 & 11.77 & 26 & ----- & 66.83 & 34\\
\hline
  10 & 100 & 14.8 & 28 & ----- &82.91 & 38\\
\hline
\end{tabular}           
\end{table}
...
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{BRAM_c}
