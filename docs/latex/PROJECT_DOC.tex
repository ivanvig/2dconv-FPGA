...
pixels and kernel values. To do this, a study about number of bits used effects
in image representation was made. 

Pixels values (\(I_{ij}\) range is 0 to 255. Dynamic range expansion
\cite{dinamic_rango} was used in order to rearrange values from 0 to 1. Kernel
...
Convolution between image and a $3\times3$ kernel results in 20 bits output
\textit{S(20,14)}. Hence, post processing is made and the outcoming result value
is shifted to positive range and truncated. Then, post-processing and 20 bits
resolution outputs are compared ir order to find out the minimum bits amount
without significative data loss. 
...
for the processing of a $1600\times1024$ image and a $3\times3$ kernel.

\section{Implementation and Results}

Figure \ref{LUT} shows the LUT usage.

\begin{figure}[!t]
\centering
\includegraphics[width=2.5in]{grafico_LUT}
\caption{LUT usage vs parallelism degree}
\label{LUT}
\end{figure}
