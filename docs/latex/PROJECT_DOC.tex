...
\begin{abstract}
  ...
  convolution implementation on a Xilinx Artix 7 FPGA platform when there is not
...
The images used for processing are grayscale because of its implementation simplicity, but the design can be extrapolated
...
the UART port to the FPGA. A batch consists of contiguous pixel columns whose dimension is described in Section ~\ref{infstorage}. In the FPGA a microprocessor receives the information
...
batch, the proposed approach is to organize the block RAM in columns. Each block RAM column corresponds to one batch
 column.
Therefore, the batch size is given
...
The AGU manages read and write memory addresses taking into account the latency from the time
a pixel is loaded into a MAC unit until the corresponding processed pixel is stored.
...
As a consequence of explained overlap, there is repeated data
...
the incoming batch is transmitted. Given $N$ MAC units and a $k\times k$ kernel, a $N+k-1$
...
System logic is implemented in the MMU
, which serves as an interface between the
memories and the rest of the components, keeping them independent from the
parallelism degree and the iteration number. An example of data-flow in memories is shown in Fig.~\ref{algorithm}.
We consider a kernel $k=3$ and $N=4$ MAC units, thus 6 memory columns are needed. In the first iteration, memories 1 to 6 
are loaded with the new batch, and the result is stored in memories 1 to 4. In the next iteration, data from a new batch 
is loaded without overwriting memories 5 and 6. After processing step, the data is stored, starting from memory 5 up to the last
...
is given by the iteratiotn number in eq.~\eqref{niter}.
...
of BRAM resources. It can be appreciated that the linear
...
parallelism degree. Table \ref{conv_tp} shows througput obtained for
different paralellism degrees.
