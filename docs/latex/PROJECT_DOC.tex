...
salida de 20 bits \(S(20,14)\).Entonces se realiza un post procesamiento donde
se lleva el resultado a un rango positivo y truncamiento. Se establece una
comparaci\'on a fines de decidir la cantidad de bits de salida del procesamiento. 

Se utiliza la relaci\'on SNR de la operaci\'on a m\'axima
resoluci\'on \(S(20,14)\) y el error pruducido al reducir la cantidad de bits
\(e_r=f(x)_{20b}-f(x)_{pos}\)\cite{srntesis}. Una representacion \(S(13,8)\) tiene una SNR
