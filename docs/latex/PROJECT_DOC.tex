...
the FPGA module. The results are shown in Fig. \ref{images_py_bo}.
...
the microblaze's C code. The client where the pre and pos-processing occurs was
written in Python 2.7.

The architecture was synthesized for different degrees of parallelism, tables
\ref{res_table_dsp} and \ref{res_table} show the resources consumed by the
module and the microprocessor on the FPGA with and without DSP respectively.
...
\caption{Resource utilization table with DSP.}
\label{res_table_dsp}
\centering
...
% if using array.sty, it might be a good idea to tweak the value of
% \extrarowheight as needed to properly center the text within the cells
\caption{Resource utilization table without DSP.}
\label{res_table}
\centering
% some packages, such as mdw tools, offer better commands for making tables
% than the plain latex2e tabular which is used here.
