...
\section{Image Processing Algorithms}\label{preproc}
...
The convolution process consists of filtering an image $I(x,y)$ of size $h
\times w$ with a set of coefficients called Kernel $K(x,y)$ as described in
\eqref{conv-org}. The mask we use in the 2D convolution is of $3 \times 3$ which
generates as output a pixel. When the filtering is done at the edges of the
image, the external pixels are completed with zeros. The process is detailed in
Fig. X.

\begin{equation}\label{conv-org}
  S(x,y) = \sum_{i=0}^{h-1} \sum_{j=0}^{w-1}K(i,j)I(x-i,y-j)
\end{equation}
...
grayscale images have a dynamic range $I_{x,y} [0,255]$. Dynamic range expansion
\cite{dinamic_rango} was used in order to rearrange values to $[0,1]$. Kernel
values $K_{x,y}$ can be negative or positive. Thus maximum norm \cite{max_norm}
was also used to displace this range to $[-1, 1]$, so that kernel effects are
maintained.

Como la finalidad del trabajo desarrollado es ser aplicado Deep Learning es habitual 
modificar rango dinamico a uno que se encuentra distubidos de sobre cero con valores positivos y negativos.
Para ello se aplican metodos para modifcar el rango a uno apto parael trabajo. Es por ello que se aplica 
la tecnica anterior.
...
the detailed transformation in Section \ref{preproc} using a Python script. In
addition, the script divides the image into batch and is sent by the UART port
to the FPGA. In the FPGA a microprocessor receives the information and
communicates it to the convolution module using a 32 bits GPIO port. 
...
k$ to $N+k-1$. That means that adding a new convolution unit only adds one new
memory columns as Fig. \ref{newconv} shows.

\begin{figure}[!t]
\centering
\includegraphics[width=3in]{new_conv}
\caption{Using shared inputs approach, only one new memory column is added for
  every new instantiated convolution unit.}
\label{newconv}
\end{figure}
...
\begin{figure*}[!t]
\centering
\subfloat[Amount of memory required.]{\includegraphics[width=2.5in]{mem_space}%
\label{mem}}
\hfil
\subfloat[Amount of data transmitted for an image of $1600\times1024$px and a $3\times3$ kernel.]{\includegraphics[width=2.5in]{data_sent}%
\label{transmitted}}
...
for processing a $1600\times1024$ image and a $3\times3$ kernel. As shown in Fig.
\ref{transmitted}, with the naive approach the transferred data is almost $3$
times the image size (because a $3\times3$ kernel) and it does not get affected
by the parallelism degree. The shared inputs approach tends to diminish the
redundant data transferred as $N$ increases, that is because the redundant data
in a $N+k-1$ batch is $k-1$ and $\frac{k-1}{N+k-1}$ tends to zero as N
increases. The shared inputs with circular shift approach is the best case
scenario where no redundant data is transferred at all.


\section{Implementation and Results}

To ensure the proper functioning of the architecture a negative filter was
coded in python and applied to an image, then the same filter was applied using
the FPGA module. The results are showed in Fig. \ref{images_py_bo}.
...
The design was implemented in an Arty 7 FPGA from Xilinx.
The architecture was synthesized for different number of convolution units. The
results obtained for BRAM utilization from the synthesis are compared with the estimated results in
Fig. \ref{bram_n}. As expected, the increase has a linear behaviour.
...
%\caption{Resource utilization table.}
%\label{res_table}
%\centering
