...
No se arranca con un simulador del sistema completo desarrolado en python ya que las cuestiones del procesamiento
...
Como primer paso se tiene una imagen destinada a procesamiento, para el cual se debe
...
rango de 8 bits, entonces surge.
...
1). Para poder tener una representaci\'on de 8 bits, para los mismos.
...
imagen a un rango conveniente de trabajo.
$$
I_N=(I-Min)\frac{newMax-newMin}{Max-Min}+newMin
$$
As\'i como en el kernel, el objetivo aqu\'i es llevar los valores de cada p\'ixel de
...
por UART se escoge como ancho de palabra 8 bits (para la etapa de env\'io de
datos).

Al obtener un cambio en el rango de la imagen que nos permite decir, que el producto de normalizar la se\~nal, en principio, el rango de los
datos cabe en 8 bits y 7 bits, al trabajar en python y al tener en cuenta que el kernel tiene componentes negativos se tiene.
\begin{itemize}
\item Representaci\'on utilizada en Imagen S(8,7)
\item Representaci\'on utilizada en Kernel S(8,7)
\end{itemize}

En la operaci\'on inicial de la convoluci\'on se obtiene como resultado en S(16,14)
por cada producto , y al tener un kernel de 3x3 se genera una suma de 9 elementos. Esto resulta en una representaci\'on S(20,14). Es decir, la salida del convolucionador
consta de 20 bits.

Por las restricciones impuestas por los m\'odulos UART, la m\'axima unidad de
informaci\'on son 8 bits, lo que implica que para enviar un solo dato , se requieren tres env\'ios.

Esto nos lleva a realizar una etapa de post procesamiento. Esta consistente en llevar el
resultado de la convoluci\'on  a rango positivo y mapear los bits menos
significativos a  uno m\'as significativo, realizando un truncado de los menos significativos.
...
o la entera, se forma una relaci\'on entre la operaci\'on a m\'axima resoluci\'on y otra
en la cual se le reducen los bits.

Se genera una relaci\'on se\~nal ruido de la estimaci\'on a 20 bits y el error
pruducido al reducir la cantidad de bits. Esto para determinar la
cantidad de bits m\'inima de trabajo.
...
Al efectuar la convoluci\'on con un filtro unitario.
$$
\begin{bmatrix}
0 & 0 & 0\\ 
0 & 1 & 0\\ 
...
\end{bmatrix}
$$
Al realizar el an\'alisis luego de reducir los bits de la parte fraccionaria. Se parte
de 8 bits en totales, en d\'onde se observa que la im\'agen mejora, y que a
partir de 13 bits totales, 1 bit de signo, 5 bits parte entera y 7 en parte
fraccionaria(figura \ref{fig_pas1})  se tiene una SNR aproximada a 30 dB, la c\'ual se considera suficiente como se observa en la figura \ref{fig_pos1}.

\begin{figure}[!t]
\centering
\includegraphics[width=3in]{posprocesamiento1.pdf}
\caption{Relaci\'on se\~nal ruido de la estimaci\'on a 20 bits y el error
pruducido al reducir la cantidad de bits.}
\label{fig_pos1}
\end{figure}

Se considera suficiente debido a observaci\'on de las im\'agenes, en d\'onde se
aprecia que si nos quedamos con 8 bits de salida (lo que intuye al principio por
las resoluciones el filtro y de la im\'agen)  se tiene una calidad muy baja. pero
a medida que se aumenta la cantidad de bits, la im\'agen mejora.

\begin{figure}[!t]
\centering
\includegraphics[width=2in]{pasted_02}
\caption{Estructura de los bits resultante.}
\label{fig_pas1}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=2in]{posprocesanto1.pdf}
\includegraphics[width=2in]{posprocesanto2.pdf}
\caption{Resultados de tomar distintos bits totales.}
\label{posprocesanto1}
\end{figure}



Al nivel de 13 bits se tiene una mejor apreciaci\'on de la im\'agen. Aunque si se
realiza un an\'alisis m\'as fino se nota una degradaci\'on respecto de la original.
...
la im\'agen queda en un segundo plano, esto se ve en la figura \ref{posprocesanto1}.

Para avanzar un paso m\'as en la reducci\'on de bits se realiza el mismo an\'alisis
al eliminar el bit m\'as significativo de la parte entera. lo cual permite quedarse
con una resoluci\'on de 12 bits como se oberva en la figura \ref{fig_pos2} y su estructura en la figura \ref{fig_pas2}.
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{posprocesamiento2}
\caption{Nueva SNR al reducir un 1 bit de la parte entera usando el mismo filtro.}
\label{fig_pos2}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=2in]{pasted_01}
\caption{Nueva estructura de los bits resultante.}
\label{fig_pas2}
\end{figure}

Se puede llevar el rango final a 8 bits, si se hace un cambio de rango igual al
que se realiz\'o para la im\'agen antes de procesarla. Para ello se requiere
conocer el m\'aximo y el m\'inimo valor de p\'ixel de la im\'agen luego de filtrar, esto
requiere que toda la im\'agen se encuentre en la memoria. Debido a las limitaci\'on
esto no se pod\'ia llevar a cabo.
